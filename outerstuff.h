...
class Pirates: public Hostile{
    ...
    void attack(int &shld, int &sec,int &e1,int &e2,int &e3,int &e4,int &reac,int &pass,int &rsk){
        numbers = rand()%20;
        ...
        if(reac>=60){
            ...
            white();
            cout << "the ship cannot use the energy disruptor due to the bad state of the ractor\n";
        }
